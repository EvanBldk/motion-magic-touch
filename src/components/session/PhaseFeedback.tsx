...
const PhaseFeedback = ({ phaseTitle, onSubmit }: Props) => {
  ...
  const handleSubmit = () => {
    onSubmit({ rpe, hasPain, painDescription, painIntensity });
  };
  ...
  return (
    <motion.div
      ...
      className="border-t border-border bg-primary/5 px-4 py-3 space-y-3"
    >
      <p className="font-oswald text-xs uppercase tracking-wider text-primary">
        Feedback — {phaseTitle}
      </p>

      <div className="space-y-1">
        <Label className="text-xs">Difficulté globale (RPE)</Label>
        <Select value={rpe} onValueChange={setRpe}>
          <SelectTrigger className="rounded-sm w-32 h-8 text-sm">
